...
function computeProgress(p) {
  ...
  const first = attempts > 0 ? firstTry/attempts : 0;
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-28 h-2 rounded bg-slate-200 overflow-hidden">
        <div className={`h-2 ${color}`} style={{ width: `${Math.max(0, Math.min(100, pct))}%` }} />
      </div>
...
export default function LearnWords({ pairs = [], onStart }) {
  ...
  const rows = useMemo(() => {
    ...
    return (pairs || [])
      ...
      .map(p => {
        ...
        return { term: p.term, def: p.def || "", ...prog, progressPct: computeProgress(prog) };
      });
  }, [pairs, getAllProgress()]);
  ...
  // переходы в игры
  const startFlashcards = () => { navigate("/flashcards"); onStart && onStart("flashcards"); };
  const startQuiz       = () => { navigate("/quiz");       onStart && onStart("quiz"); };
    ...
    >
      ...
      <div className="max-w-7xl -ml-8 mr-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <button type="button" className={"btn " + (filter==="unlearned"  ? "btn-primary" : "")} onClick={()=>setFilter("unlearned")}>Need practice</button>
          <button type="button" className={"btn " + (filter==="low"        ? "btn-primary" : "")} onClick={()=>setFilter("low")}>&lt; 50% progress</button>
          <button type="button" className={"btn " + (filter==="learned"    ? "btn-primary" : "")} onClick={()=>setFilter("learned")}>Learned</button>
        </div>
        ...
        <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-200">
                ...
                <tr key={r.term}>
                  ...
                  <td className="px-4 py-3">
                    {r.learned ? (
                      ...
                      <span className="inline-flex items-center px-3 py-1 rounded bg-amber-50 text-amber-700 border border-amber-200">No</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td className="px-4 py-6 text-center text-slate-500" colSpan={9}>Nothing to show.</td>
                </tr>
              )}
            </tbody>
          </table>
